...
async function writeDB(data: any) {
  ...
  if (!process.env.VERCEL) {
    try {
      ...
      console.warn("Could not save to disk, running in memory only");
    }
  }
}

// Generate an auto-incrementing ID for a table given a list
function getNextId(tableArray: any[]) {
  return tableArray.length > 0 ? Math.max(...tableArray.map((item: any) => item.id)) + 1 : 1;
}
...
app.get('/api/dashboard', async (req, res) => {
  try {
    ...
    const inProduction = (db.orders || []).filter((o: any) => o.status === 'in_production').length;
    const delayedOrders = (db.orders || []).filter((o: any) => o.status !== 'delivered' && o.deadline && new Date(o.deadline) < new Date()).length;
    const totalClients = (db.clients || []).length;
    
    const revenue = (db.quotations || [])
      .filter((q: any) => q.status === 'accepted' || q.status === 'approved')
      .reduce((sum: number, q: any) => sum + (q.total_amount || q.price || 0), 0);
...
app.get('/api/quotations', async (req, res) => {
  try {
    ...
    quotesWithClient.sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
...
app.put('/api/quotations/:id/status', async (req, res) => {
  try {
    ...
    const db = await readDB();
    const quoteIndex = db.quotations.findIndex((q: any) => q.id === parseInt(req.params.id, 10));
...
app.post('/api/orders/convert/:quotationId', async (req, res) => {
  try {
    ...
    const quoteIndex = db.quotations.findIndex((q: any) => q.id === quotationId);
    ...
    const newOrder = {
      ...
      product_details: quote.items ? quote.items.map((i: any) => i.description).join(', ') : (quote.product_type || 'Converted Quote'),
      quantity: quote.items ? quote.items.reduce((sum: number, i: any) => sum + (parseInt(i.quantity, 10)||0), 0) : (quote.quantity || 1),
...
app.put('/api/orders/:id/status', async (req, res) => {
  try {
    const { status } = req.body;
    const db = await readDB();
    const orderIndex = db.orders.findIndex((o: any) => o.id === parseInt(req.params.id, 10));
...
app.post('/api/production/work-orders/:id/complete', async (req, res) => {
  try {
    ...
    const db = await readDB();
    const woIndex = (db.work_orders || []).findIndex((wo: any) => wo.id === parseInt(req.params.id, 10));
    ...
    const bom = (db.boms || []).find((b: any) => b.id === wo.bom_id);
    ...
    for (const comp of bom.components) {
      ...
      const invIndex = db.inventory.findIndex((i: any) => i.sku === comp.sku);
      if (invIndex !== -1) {
    ...
    const fgIndex = db.inventory.findIndex((i: any) => i.sku === bom.finished_sku);
    if (fgIndex !== -1) {
    ...
    if (status === 'in_progress') {
      ...
      if (bom) {
        ...
        for (const comp of bom.components) {
          ...
          const invItem = (db.inventory || []).find((i: any) => i.sku === comp.sku);
...
app.put('/api/quality/:id', async (req, res) => {
  try {
    ...
    const db = await readDB();
    const checkIndex = (db.quality_checks || []).findIndex((q: any) => q.id === parseInt(req.params.id, 10));
...
app.post('/api/procurement/pos/:id/receive', async (req, res) => {
  try {
    ...
    const invIndex = db.inventory.findIndex((i: any) => i.sku === po.required_sku);
    if (invIndex !== -1) {
